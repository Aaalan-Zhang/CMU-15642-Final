...
num_epochs = 1  # run just 1 epoch with pruning
...
class PruningTrainer(CrossEncoderTrainer):
    def __init__(self, *args, ffn_norm=2, attn_norm=2, **kwargs):
        ...
        self.schedule = {
            150:  ("ffn",  "all",   0.1),    # FFN prune #1
            200:  ("ffn",[0,1,2],   0.15),   # Attn prune #1 (layers 0–1)
            300:  ("ffn",  [3,4,5],   0.15),    # FFN prune #2
            350:  ("ffn",[6,7,8],   0.15),   # Attn prune #2 (layers 4–5)
            450:  ("ffn",  [9,10,11],   0.15),    # FFN prune #3
            550: ("ffn","all",   0.1),   # Attn prune #3 (layers 8–9)
        }
