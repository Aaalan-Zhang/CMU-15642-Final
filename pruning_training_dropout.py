...
class PruningTrainer(CrossEncoderTrainer):
    def __init__(self, *args, weight_dropout: float = 0.1, **kwargs):
        super().__init__(*args, **kwargs)
        self.weight_dropout = weight_dropout
        self.step_counter = 0
        self.masks = []

        # 1) Create a mask for each layer
    ...
    def training_step(self, model: CrossEncoder, inputs: dict, batch_size: int) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            for blk, mask in zip(model.bert.encoder.layer, self.masks):
                blk.intermediate.dense.weight.data.mul_(mask)

            # 3) every 100 steps, expand the mask by dropping a random subset of *remaining* weights
            if self.step_counter % 100 == 0 and self.step_counter <=500:
                print(f"— Pruning at step {self.step_counter} —")
                new_masks = []
                for blk, old_mask in zip(model.bert.encoder.layer[:2], self.masks[:2]):
                    w = blk.intermediate.dense.weight
                ...
                self.masks[0] = new_masks[0]
                self.masks[1] = new_masks[1]
